...
__global__ void runLGCA(unsigned int* b1, unsigned int* b2, unsigned int* b3, unsigned int* b4, unsigned int* b5, unsigned int* b6, int m, int n)
{
	...
	bool a=(as[linidx] ? true : false);
	bool b=(bs[linidx] ? true : false);
	bool c=(cs[linidx] ? true : false);
	bool d=(ds[linidx] ? true : false);
	bool e=(es[linidx] ? true : false);
	bool f=(fs[linidx] ? true : false);

	// Two body collision
	bool twoCol = ((a&d&~(b|c|e|f)) | (b&e&~(a|c|d|f)) | (c&f&~(a|b|d|e)));
	
	// Three body collision
	bool threeCol = (a^b) & (b^c) & (c^d) & (d^e) & (e^f);

	if (twoCol|threeCol)
	{ 
		//Rotate
		//Direction selection - naive method
